...
const client = new Client({ //Вставил свои параметры из БД
    user: 'postgres',
    host: 'localhost', 
    database: 'lib',
    password: '12345',
...
app.post('/signin', async (req, res) => {
    ...
    let sql = {
        text: "SELECT name as result FROM users WHERE name = $1 AND pass = $2", // Убрал шифровку с помощью md5 из-за нее неработало))
        values: [login, pass]
    };
    try{
        let data = await client.query(sql);
